...
1. **Scan** — `ds2a2ui scan` drafts `a2ui.manifest.json` and a review report from TypeScript sources.
2. **Curate** — edit the manifest allowlist (`includeInCatalog`, props, descriptions).
3. **Validate** — `ds2a2ui validate` checks schema and diagnostics.
4. **Convert** — `ds2a2ui convert` writes the A2UI catalog and supporting files (offline, no LLM).
5. **Integrate** — client and agent agree on `catalogId`; agent UI renders with your existing components.

See **[docs/manifest.md](docs/manifest.md)** for the full manifest workflow.
...
npx tsx src/cli.ts scan examples/acme-ds
npx tsx src/cli.ts validate examples/acme-ds
...
```bash
npx tsx src/cli.ts scan /path/to/your-ds --project /path/to/tsconfig.json
...
- Optional `a2ui.manifest.json` — generated by `ds2a2ui scan` or hand-written for an explicit inventory ([docs/manifest.md](docs/manifest.md))
